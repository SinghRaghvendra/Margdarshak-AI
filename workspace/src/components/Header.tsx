...
import {
  Menu, Home, Info, DollarSign, Mail,
  LogIn, UserPlus, LogOut, BookUser, User as UserIcon
} from 'lucide-react';
...
export default function Header() {
  ...
  const loggedInNavItems = [
    ...mainNavItems,
    { label: 'My Reports', href: '/my-reports', icon: <BookUser className="mr-2 h-5 w-5" /> },
    { label: 'My Profile', href: '/profile', icon: <UserIcon className="mr-2 h-5 w-5" /> },
  ];
      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-1">
          ...
          {user ? (
            ...
            <>
              <Link href="/login">
                <Button variant="ghost" className="text-sm text-muted-foreground hover:text-foreground px-3 py-2">Login</Button>
              </Link>
              <Link href="/signup">
                <Button className="text-sm ml-2 px-4 py-2">Sign Up</Button>
              </Link>
            </>
          )}
        </nav>
