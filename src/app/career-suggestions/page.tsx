...
export default function CareerSuggestionsPage() {
  ...
  const handleProceedToPayment = async () => {
    if (selectedCareers.length === 0) {
      toast({ title: `Please select at least 1 career`, description: `You have selected none.`, variant: 'destructive'});
      return;
    }
    if (!user) {
  ...
  return (
    <div className="py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-md text-primary mt-2">Please select up to 3 choices to generate detailed reports.</p>
      </div>
      ...
      <Alert variant="destructive" className="max-w-2xl mx-auto mb-8">
        ...
        <AlertDescription>
            The career suggestions provided are generated by an AI and should be used as a starting point for your own research and consideration. This is not a substitute for professional advice.
        </AlertDescription>
      </Alert>
      ...
      {!isLoading && !generationError && allSuggestions && allSuggestions.length > 0 && (
        <>
          ...
          <div className="mt-10 text-center">
            ...
            >
              Proceed with {selectedCareers.length} selection(s) <ArrowRight className="ml-2 h-5 w-5" />
            </Button>
            {selectedCareers.length === 0 && (
                 <p className="text-sm text-muted-foreground mt-2">Please select at least 1 career to continue.</p>
            )}
          </div>
        </>
      )}
